...
std::default_random_engine g_generator(static_cast<int>(
...
CudaTensor::CudaTensor(int d0, int d1, int d2, int d3)
    : _dataGpu(nullptr), _dataGpuSize(0) {
...
void CudaTensor::SetDropoutMask(float zeroRatio) {
  ...
  for (int i = 0; i < _dataSize; ++i) {
    _data[i] = 0.0f;
    if (g_uniformDistribution(g_generator) > zeroRatio) _data[i] = s;
  }
  PushToGpu();
}

void CudaTensor::PushToGpu() {
  cudaError_t err = cudaMemcpy(_dataGpu, &_data[0], _dataSize * sizeof(float),
                               cudaMemcpyKind::cudaMemcpyHostToDevice);
  assert(err == cudaSuccess);
}

void CudaTensor::PullFromGpu() {
  cudaError_t err = cudaMemcpy(&_data[0], _dataGpu, _dataSize * sizeof(float),
...
__global__ void LinearTransform_Cuda(float* y, const float* x, const float* w,
                                     const float* b, int nColX, int nColW,
                                     int nJobs) {
...
const CudaTensor* FcLayer::Forward(const CudaTensor* x) {
  ...
  LinearTransform_Cuda<<<block, threads>>>(_y._dataGpu, _pX->_dataGpu,
                                           _w._dataGpu, _b._dataGpu, x_d0,
                                           _w._d0, nJobs);
  assert(cudaGetLastError() == cudaSuccess);
  return &_y;
}

__global__ void BackwardFc_Wg_Cuda(float* wG, const float* x, const float* yG,
                                   int nXcol, int nXrow, int nWcol, int nJobs) {
...
__global__ void BackwardFc_Bg_Cuda(float* bG, const float* yG, int nYgCol,
                                   int nYgRow) {
...
__global__ void BackwardFc_Xg_Cuda(float* xG, const float* yG, const float* w,
                                   int yGw, int wTh, int xGw, int nJobs) {
...
const CudaTensor* FcLayer::Backward(const CudaTensor* yG,
...

  {
    ...
    int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
    BackwardFc_Wg_Cuda<<<block, threads>>>(
        _wG._dataGpu, _pX->_dataGpu, yG->_dataGpu, x_d0, x_d1, _wG._d0, nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
  {
    int numBlocks = (_b._d0 + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
    BackwardFc_Bg_Cuda<<<block, threads>>>(_bG._dataGpu, yG->_dataGpu, yG->_d0,
                                           yG->_d1);
    assert(cudaGetLastError() == cudaSuccess);
  }
  ...
  if (layerIndex > 0) {
    ...
    int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
    BackwardFc_Xg_Cuda<<<block, threads>>>(_xG._dataGpu, yG->_dataGpu,
                                           _w._dataGpu, yG->_d0, _w._d0,
                                           _xG._d0, nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
  return &_xG;
...
__global__ void UpdateWs_Cuda(float learningRate, float beta1, float beta2,
                              float beta1t, float beta2t, float* w,
                              const float* wG, float* wG_m, float* wG_v,
                              int nJobs) {
...
void FcLayer::UpdateWs(float learningRate, float beta1, float beta2,
                       float beta1t, float beta2t) {
  {
    int nJobs = _w._d1 * _w._d0;
    int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
    UpdateWs_Cuda<<<block, threads>>>(learningRate, beta1, beta2, beta1t,
                                      beta2t, _w._dataGpu, _wG._dataGpu,
                                      _wG_m._dataGpu, _wG_v._dataGpu, nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
  {
    int numBlocks = (_b._d0 + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
    UpdateWs_Cuda<<<block, threads>>>(learningRate, beta1, beta2, beta1t,
                                      beta2t, _b._dataGpu, _bG._dataGpu,
...
Conv2dLayer::Conv2dLayer(CudaNn* nn, int kernelSize, int nInChannel,
                         int nOutChannel, int stride, int padding)
    : CudaLayer(nn),
...
__global__ void ForwardConv2d_Cuda(float* y, const float* x, const float* w,
                                   const float* b, int nOutChannel, int nRowY,
                                   int nColY, int nInChannel, int nRowX,
                                   int nColX, int kernelSize, int stride,
                                   int padding, int nJobs) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index >= nJobs) return;
  ...
  for (int rowX = startRowX + startRowKernel; rowX < startRowX + kernelSize;
       ++rowX) {
    if (rowX >= nRowX) break;
    for (int colX = startColX + startColKernel; colX < startColX + kernelSize;
         ++colX) {
      if (colX >= nColX) break;
      for (int inChannel = 0; inChannel < nInChannel; ++inChannel) {
        int xBaseIndex = image * (nInChannel * nRowX * nColX) +
                         inChannel * (nRowX * nColX) + rowX * nColX;
        int wBaseIndex = outChannel * (nInChannel * kernelSize * kernelSize) +
                         inChannel * (kernelSize * kernelSize) +
                         (rowX - startRowX) * kernelSize;
...
const CudaTensor* Conv2dLayer::Forward(const CudaTensor* x) {
  ...
  ForwardConv2d_Cuda<<<blocks, threads>>>(
      _y._dataGpu, _pX->_dataGpu, _w._dataGpu, _b._dataGpu, _y._d2, _y._d1,
      _y._d0, _pX->_d2, _pX->_d1, _pX->_d0, _kernelSize, _stride, _padding,
      nJobs);
  assert(cudaGetLastError() == cudaSuccess);
  return &_y;
}

__global__ void BackwardConv2d_Wg_Cuda(float* wG, const float* x,
                                       const float* yG, int nOutChannel,
                                       int nInChannel, int nImages, int nRowY,
                                       int nColY, int nRowX, int nColX,
  ...
  for (int image = 0; image < nImages; ++image) {
    int yBaseIndex =
        image * (nOutChannel * nRowY * nColY) + outChannel * (nRowY * nColY);
    int xBaseIndex =
        image * (nInChannel * nRowX * nColX) + inChannel * (nRowX * nColX);
...
__global__ void BackwardConv2d_Bg_Cuda(float* bG, const float* yG, int nImages,
  ...
  for (int image = 0; image < nImages; ++image) {
    for (int rowY = 0; rowY < nRowY; ++rowY) {
      int yBaseIndex = image * (nOutChannel * nRowY * nColY) +
                       outChannel * (nRowY * nColY) + rowY * nColY;
...
__global__ void BackwardConv2d_Xg_Cuda(float* xG, const float* w,
                                       const float* yG, int nImages,
  ...
  int startRowY =
      max(0, int(floor(float(rowX + padding - kernelSize) / stride)) + 1);
  int startColY =
      max(0, int(floor(float(colX + padding - kernelSize) / stride)) + 1);
  for (int rowY = startRowY; rowY < nRowY; ++rowY) {
    ...
    for (int colY = startColY; colY < nColY; ++colY) {
      ...
      for (int outChannel = 0; outChannel < nOutChannel; ++outChannel) {
        int yIndex = image * (nOutChannel * nRowY * nColY) +
                     outChannel * (nRowY * nColY) + rowY * nColY + colY;
        int wIndex = outChannel * (nInChannel * kernelSize * kernelSize) +
                     inChannel * (kernelSize * kernelSize) + rowW * kernelSize +
                     colW;
...
const CudaTensor* Conv2dLayer::Backward(const CudaTensor* yG,
                                        const int layerIndex) {
...

  {
    ...
    int numBlocks =
        (nJobs + K_SMALL_THREAD_PER_BLOCK - 1) / K_SMALL_THREAD_PER_BLOCK;
    dim3 blocks(numBlocks), threads(K_SMALL_THREAD_PER_BLOCK);
    BackwardConv2d_Wg_Cuda<<<blocks, threads>>>(
        _wG._dataGpu, _pX->_dataGpu, yG->_dataGpu, _wG._d3, _wG._d2, _y._d3,
        _y._d1, _y._d0, _pX->_d1, _pX->_d0, _kernelSize, _stride, _padding,
        nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
  {
    ...
    int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 blocks(numBlocks), threads(K_THREAD_PER_BLOCK);
    BackwardConv2d_Bg_Cuda<<<blocks, threads>>>(_bG._dataGpu, yG->_dataGpu,
                                                _y._d3, _y._d2, _y._d1, _y._d0);
    assert(cudaGetLastError() == cudaSuccess);
  }
  if (layerIndex > 0) {
    ...
    BackwardConv2d_Xg_Cuda<<<blocks, threads>>>(
        _xG._dataGpu, _w._dataGpu, yG->_dataGpu, _xG._d3, _xG._d2, _xG._d1,
        _xG._d0, _y._d2, _y._d1, _y._d0, _kernelSize, _stride, _padding, nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
  return &_xG;
}

void Conv2dLayer::UpdateWs(float learningRate, float beta1, float beta2,
                           float beta1t, float beta2t) {
  {
...
__global__ void ForwardMaxPool_Cuda(float* y, float* maxIndex, const float* x,
                                    int nImages, int nInChannel, int nRow,
                                    int nCol) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  const int offset[4][4] = {0,
                            ...
                            nCol * 2};
  int s = nJobs % 4;
  int xIndex = image * (nInChannel * nRow * nCol * 4) +
               inChannel * (nRow * nCol * 4) + row * nCol * 4 + col * 2;
...
const CudaTensor* MaxPoolLayer::Forward(const CudaTensor* x) {
  ...
  int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
  dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
  ForwardMaxPool_Cuda<<<block, threads>>>(_y._dataGpu, _maxIndex._dataGpu,
                                          _pX->_dataGpu, _y._d3, _y._d2, _y._d1,
                                          _y._d0);
  assert(cudaGetLastError() == cudaSuccess);
  return &_y;
}

__global__ void BackwardMaxPool_Cuda(float* xG, const float* yG,
                                     const float* maxIndex, int nImages,
                                     int nOutChannel, int nRow, int nCol) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  int xGindex = image * (nOutChannel * nRow * nCol * 4) +
                outChannel * (nRow * nCol * 4) + row * nCol * 4 + col * 2;
  xG[xGindex] = 0.0f;
  xG[xGindex + 1] = 0.0f;
...
const CudaTensor* MaxPoolLayer::Backward(const CudaTensor* yG,
                                         const int layerIndex) {
  assert(_y._dataSize == yG->_dataSize);
  ...
  if (layerIndex > 0) {
    ...
    int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
    BackwardMaxPool_Cuda<<<block, threads>>>(_xG._dataGpu, yG->_dataGpu,
                                             _maxIndex._dataGpu, _y._d3, _y._d2,
                                             _y._d1, _y._d0);
    assert(cudaGetLastError() == cudaSuccess);
  }
  return &_xG;
...
__global__ void BackwardRelu_Cuda(float* xG, const float* yG, const float* x,
                                  int nJobs) {
  int jobIndex = blockIdx.x * blockDim.x + threadIdx.x;
  if (jobIndex >= nJobs) return;
...
const CudaTensor* ReluLayer::Backward(const CudaTensor* yG,
                                      const int layerIndex) {
  if (layerIndex > 0) {
    ...
    int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
    BackwardRelu_Cuda<<<block, threads>>>(_xG._dataGpu, yG->_dataGpu,
                                          _pX->_dataGpu, nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
  return &_xG;
...
BatchNorm2dLayer::BatchNorm2dLayer(CudaNn* nn, int inDim)
    : CudaLayer(nn), _pX(nullptr), _accCount(0) {
...
__global__ void ForwardBatchNorm2d_Train_Cuda(float* meanAndVariance,
                                              float* meanAndVarianceAcc,
                                              float* xHat, float* y,
...
__global__ void ForwardBatchNorm2d_Cuda(float* y,
                                        const float* meanAndVarianceAcc,
                                        int accCount, const float* w,
                                        const float* x, int nRow, int nCol) {
  int nChannel = gridDim.x;
  int ch = blockIdx.x;
  int image = threadIdx.x;
...
const CudaTensor* BatchNorm2dLayer::Forward(const CudaTensor* x) {
  ...
  if (_nn->IsTraining()) {
    ...
    ForwardBatchNorm2d_Train_Cuda<256><<<x->_d2, x->_d3>>>(
        _meanAndVariance._dataGpu, _meanAndVarianceAcc._dataGpu, _xHat._dataGpu,
        _y._dataGpu, _w._dataGpu, x->_dataGpu, x->_d1, x->_d0);
    assert(cudaGetLastError() == cudaSuccess);
    ++_accCount;
  } else {
    assert(x->_d3 <= 256);
    ForwardBatchNorm2d_Cuda<<<x->_d2, x->_d3>>>(
        _y._dataGpu, _meanAndVarianceAcc._dataGpu,
        _accCount > 1 ? _accCount : 2, _w._dataGpu, x->_dataGpu, x->_d1,
        x->_d0);
    assert(cudaGetLastError() == cudaSuccess);
  }
...
template <int BLOCK_SIZE>
__global__ void BackwardBatchNorm2d_Cuda(
    float* wG, float* xG, float* meanAndVarianceG, float* meanAndVarianceAcc,
    const float* w, const float* x, const float* xHat,
    const float* meanAndVariance, const float* yG, int nRow, int nCol) {
  int nChannel = gridDim.x;
  int nImages = blockDim.x;
  int ch = blockIdx.x;
  int image = threadIdx.x;
  ...
  for (int i = 0; i < channelStride; ++i) {
    meanGarr[image] +=
        xG[baseIndex + i] * b +
        varianceG * ((-2.0f * (x[baseIndex + i] - mean)) / mDash);
  }
  __syncthreads();
  float meanG = 0.0f;
  for (int i = 0; i < nImages; ++i) {
  ...
  for (int i = 0; i < channelStride; ++i) {
    ...
    xG[baseIndex + i] =
        currXg * varianceSqrtInv +
        varianceG * ((2.0f * (x[baseIndex + i] - mean)) / mDash) + b;
  }
...
const CudaTensor* BatchNorm2dLayer::Backward(const CudaTensor* yG,
                                             const int layerIndex) {
  ...
  BackwardBatchNorm2d_Cuda<256><<<_xG._d2, _xG._d3>>>(
      ...
      _meanAndVarianceAcc._dataGpu, _w._dataGpu, _pX->_dataGpu, _xHat._dataGpu,
      _meanAndVariance._dataGpu, yG->_dataGpu, _xG._d1, _xG._d0);
  assert(cudaGetLastError() == cudaSuccess);
  _accCount = 0;
  return &_xG;
}

void BatchNorm2dLayer::UpdateWs(float learningRate, float beta1, float beta2,
                                float beta1t, float beta2t) {
  {
    int nJobs = _w._d1 * _w._d0;
    int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
...
__global__ void Dropout_Cuda(float* x, const float* inputX,
                             const float* dropoutMask, int nJobs) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index >= nJobs) return;
...
const CudaTensor* DropoutLayer::Forward(const CudaTensor* x) {
  ...
  int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
  dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
  Dropout_Cuda<<<block, threads>>>(_xDropped._dataGpu, x->_dataGpu,
                                   _dropoutMask._dataGpu, nJobs);
  assert(cudaGetLastError() == cudaSuccess);
  return &_xDropped;
}

const CudaTensor* DropoutLayer::Backward(const CudaTensor* yG,
                                         const int layerIndex) {
  ...
  if (layerIndex > 0) {
    ...
    int numBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 block(numBlocks), threads(K_THREAD_PER_BLOCK);
    Dropout_Cuda<<<block, threads>>>(_yGdropped._dataGpu, yG->_dataGpu,
                                     _dropoutMask._dataGpu, nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
  return &_yGdropped;
...
__global__ void ForwardQuatNorm_Cuda(float* y, const float* x, int nQuats,
                                     int nJobs) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index >= nJobs) return;
  ...
  float q[4] = {x[baseIndex], x[baseIndex + 1], x[baseIndex + 2],
                x[baseIndex + 3]};
  float l =
      sqrtf(q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3]) + 1e-8f;
  y[baseIndex + 0] = q[0] / l;
  y[baseIndex + 1] = q[1] / l;
  y[baseIndex + 2] = q[2] / l;
  y[baseIndex + 3] = q[3] / l;
...
const CudaTensor* QuatNormLayer::Forward(const CudaTensor* x) {
  ...
  ForwardQuatNorm_Cuda<<<block, threads>>>(_y._dataGpu, _pX->_dataGpu,
                                           _pX->_d1 / 4, nJobs);
  assert(cudaGetLastError() == cudaSuccess);
...
__global__ void BackwardQuatNorm_Cuda(float* xG, const float* x,
                                      const float* yG, int nQuats, int nJobs) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index >= nJobs) return;
  ...
  float q[4] = {x[baseIndex], x[baseIndex + 1], x[baseIndex + 2],
                x[baseIndex + 3]};
  float tYg[4] = {yG[baseIndex], yG[baseIndex + 1], yG[baseIndex + 2],
                  yG[baseIndex + 3]};
...
const CudaTensor* QuatNormLayer::Backward(const CudaTensor* yG,
                                          const int layerIndex) {
  ...
  if (layerIndex > 0) {
    ...
    int nBlocks =
        (nJobs + K_SMALL_THREAD_PER_BLOCK - 1) / K_SMALL_THREAD_PER_BLOCK;
    dim3 block(nBlocks), threads(K_SMALL_THREAD_PER_BLOCK);
    BackwardQuatNorm_Cuda<<<block, threads>>>(_xG._dataGpu, _pX->_dataGpu,
                                              yG->_dataGpu, _xG._d0 / 4, nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
...
__global__ void ForwardSoftmax_Cuda(float* softmax, const float* x, int nRow,
                                    int nCol) {
...
const CudaTensor* SoftmaxLayer::Forward(const CudaTensor* x) {
  ...
  ForwardSoftmax_Cuda<<<block, threads>>>(_softmax._dataGpu, x->_dataGpu,
                                          x->_d1, x->_d0);
  assert(cudaGetLastError() == cudaSuccess);
  return &_softmax;
}

__global__ void BackwardSoftmax_Cuda(float* lossG, const float* softmax,
                                     const float* yLabel, int nCol, int nJobs) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index >= nJobs) return;
  int r = index / nCol;
  int c = index % nCol;
...
const CudaTensor* SoftmaxLayer::Backward(const CudaTensor* yG,
                                         const int layerIndex) {
  ...
  if (layerIndex > 0) {
    ...
    dim3 block(nBlocks), threads(K_THREAD_PER_BLOCK);
    BackwardSoftmax_Cuda<<<block, threads>>>(_lossG._dataGpu, _softmax._dataGpu,
                                             yG->_dataGpu, _lossG._d0, nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
  return &_lossG;
}
...
__global__ void BackwardSumOfSqures(float* yG, const float* y,
                                    const float* yLabel, int nJobs) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  if (index >= nJobs) return;
...
const CudaTensor* SumOfSquaresLayer::Backward(const CudaTensor* yLabel,
                                              const int layerIndex) {
  ...
  if (layerIndex > 0) {
    ...
    int nBlocks = (nJobs + K_THREAD_PER_BLOCK - 1) / K_THREAD_PER_BLOCK;
    dim3 block(nBlocks), threads(K_THREAD_PER_BLOCK);
    BackwardSumOfSqures<<<block, threads>>>(_yG._dataGpu, _pY->_dataGpu,
                                            yLabel->_dataGpu, nJobs);
    assert(cudaGetLastError() == cudaSuccess);
  }
  return &_yG;
}

void SumOfSquaresLayer::Pull() { _yG.PullFromGpu(); }

///////////////////////////////////////////////////////////////////////
CudaNn::CudaNn() : _beta1t(kBeta1), _beta2t(kBeta2), _train(false) {}

CudaNn::~CudaNn() { InitializeCudaNn(""); }
...
bool CudaNn::AddConv2d(int kernelSize, int nInChannel, int nOutChannel,
                       int stride, int padding) {
  _layers.push_back(new Conv2dLayer(this, kernelSize, nInChannel, nOutChannel,
                                    stride, padding));
  return true;
}
